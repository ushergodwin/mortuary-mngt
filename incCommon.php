...
function getTableList($skip_authentication = false)
{
	...
	$arrTables = array(
		...
		'incoming_deceased' => array('Incoming deceased', '', 'resources/table_icons/application_side_expand.png', 'None'),
		'outgoing_deceased' => array('Outgoing deceased', '', 'resources/table_icons/application_side_contract.png', 'None'),
		'relatives_info' => array('Relatives info', '', 'resources/table_icons/group.png', 'None'),
...
{
	$sql_fields = array(
		'incoming_deceased' => "`incoming_deceased`.`id` as 'id', `incoming_deceased`.`fullname` as 'fullname', `incoming_deceased`.`gender` as 'gender', `incoming_deceased`.`tag_number` as 'tag_number', `incoming_deceased`.`serial_number` as 'serial_number', IF(    CHAR_LENGTH(`relatives_info1`.`first_relative_full_name`), CONCAT_WS('',   `relatives_info1`.`first_relative_full_name`), '') as 'relative_name', IF(    CHAR_LENGTH(`relatives_info1`.`phone_number`), CONCAT_WS('',   `relatives_info1`.`phone_number`), '') as 'relative_number', IF(    CHAR_LENGTH(`rooms1`.`name`), CONCAT_WS('',   `rooms1`.`name`), '') as 'room', IF(    CHAR_LENGTH(`beds1`.`number`) || CHAR_LENGTH(`rooms2`.`name`), CONCAT_WS('',   `beds1`.`number`, ' Room: ', `rooms2`.`name`), '') as 'bed', if(`incoming_deceased`.`date`,date_format(`incoming_deceased`.`date`,'%m/%d/%Y'),'') as 'date'",
		'outgoing_deceased' => "`outgoing_deceased`.`id` as 'id', IF(    CHAR_LENGTH(`incoming_deceased1`.`fullname`), CONCAT_WS('',   `incoming_deceased1`.`fullname`), '') as 'fullname', IF(    CHAR_LENGTH(`incoming_deceased1`.`gender`), CONCAT_WS('',   `incoming_deceased1`.`gender`), '') as 'gender', IF(    CHAR_LENGTH(`incoming_deceased1`.`tag_number`), CONCAT_WS('',   `incoming_deceased1`.`tag_number`), '') as 'tag_number', IF(    CHAR_LENGTH(`incoming_deceased1`.`serial_number`), CONCAT_WS('',   `incoming_deceased1`.`serial_number`), '') as 'serial_number', IF(    CHAR_LENGTH(`relatives_info1`.`first_relative_full_name`), CONCAT_WS('',   `relatives_info1`.`first_relative_full_name`), '') as 'relative_name', IF(    CHAR_LENGTH(`relatives_info1`.`phone_number`), CONCAT_WS('',   `relatives_info1`.`phone_number`), '') as 'relative_phone_number', if(`outgoing_deceased`.`date`,date_format(`outgoing_deceased`.`date`,'%m/%d/%Y'),'') as 'date', `outgoing_deceased`.`car_out_number` as 'car_out_number'",
		'relatives_info' => "`relatives_info`.`id` as 'id', `relatives_info`.`first_relative_full_name` as 'first_relative_full_name', `relatives_info`.`home_address` as 'home_address', `relatives_info`.`home_town` as 'home_town', `relatives_info`.`occupation` as 'occupation', `relatives_info`.`phone_number` as 'phone_number', `relatives_info`.`second_relative_full_name` as 'second_relative_full_name', `relatives_info`.`second_relative_home_address` as 'second_relative_home_address', `relatives_info`.`second_relative_home_town` as 'second_relative_home_town', `relatives_info`.`second_relative_occupation` as 'second_relative_occupation', `relatives_info`.`second_relative_phone_number` as 'second_relative_phone_number'",
		'rooms' => "`rooms`.`id` as 'id', `rooms`.`name` as 'name', `rooms`.`type` as 'type', `rooms`.`status` as 'status'",
		'beds' => "`beds`.`id` as 'id', `beds`.`number` as 'number', IF(    CHAR_LENGTH(`rooms1`.`name`), CONCAT_WS('',   `rooms1`.`name`), '') as 'room', `beds`.`status` as 'status'",
		'bill' => "`bill`.`id` as 'id', IF(    CHAR_LENGTH(`incoming_deceased1`.`fullname`) || CHAR_LENGTH(`incoming_deceased1`.`tag_number`), CONCAT_WS('',   `incoming_deceased1`.`fullname`, '  TagNo:', `incoming_deceased1`.`tag_number`), '') as 'deceased', IF(    CHAR_LENGTH(`invoices1`.`total`), CONCAT_WS('',   `invoices1`.`total`), '') as 'total', `bill`.`paid_amount` as 'paid_amount', IF(    CHAR_LENGTH(`invoices1`.`balance`), CONCAT_WS('',   `invoices1`.`balance`), '') as 'balance'",
		'invoices' => "`invoices`.`id` as 'id', IF(    CHAR_LENGTH(`incoming_deceased1`.`fullname`) || CHAR_LENGTH(`incoming_deceased1`.`tag_number`), CONCAT_WS('',   `incoming_deceased1`.`fullname`, '  TagNo:', `incoming_deceased1`.`tag_number`), '') as 'deceased', IF(    CHAR_LENGTH(`beds1`.`number`) || CHAR_LENGTH(`rooms1`.`name`), CONCAT_WS('',   `beds1`.`number`, ' Room: ', `rooms1`.`name`), '') as 'room', IF(    CHAR_LENGTH(`relatives_info1`.`first_relative_full_name`) || CHAR_LENGTH(`relatives_info1`.`phone_number`), CONCAT_WS('',   `relatives_info1`.`first_relative_full_name`, '  Phone: ', `relatives_info1`.`phone_number`), '') as 'relative', `invoices`.`services` as 'services', `invoices`.`total` as 'total', `invoices`.`balance` as 'balance', if(`invoices`.`date`,date_format(`invoices`.`date`,'%m/%d/%Y'),'') as 'date'"
	);
...
{
	$sql_from = array(
		'incoming_deceased' => "`incoming_deceased` LEFT JOIN `relatives_info` as relatives_info1 ON `relatives_info1`.`id`=`incoming_deceased`.`relative_name` LEFT JOIN `rooms` as rooms1 ON `rooms1`.`id`=`incoming_deceased`.`room` LEFT JOIN `beds` as beds1 ON `beds1`.`id`=`incoming_deceased`.`bed` LEFT JOIN `rooms` as rooms2 ON `rooms2`.`id`=`beds1`.`room` ",
		'outgoing_deceased' => "`outgoing_deceased` LEFT JOIN `incoming_deceased` as incoming_deceased1 ON `incoming_deceased1`.`id`=`outgoing_deceased`.`fullname` LEFT JOIN `relatives_info` as relatives_info1 ON `relatives_info1`.`id`=`incoming_deceased1`.`relative_name` ",
		'relatives_info' => "`relatives_info` ",
		'rooms' => "`rooms` ",
		'beds' => "`beds` LEFT JOIN `rooms` as rooms1 ON `rooms1`.`id`=`beds`.`room` ",
		'bill' => "`bill` LEFT JOIN `invoices` as invoices1 ON `invoices1`.`id`=`bill`.`deceased` LEFT JOIN `incoming_deceased` as incoming_deceased1 ON `incoming_deceased1`.`id`=`invoices1`.`deceased` ",
		'invoices' => "`invoices` LEFT JOIN `incoming_deceased` as incoming_deceased1 ON `incoming_deceased1`.`id`=`invoices`.`deceased` LEFT JOIN `beds` as beds1 ON `beds1`.`id`=`incoming_deceased1`.`bed` LEFT JOIN `rooms` as rooms1 ON `rooms1`.`id`=`beds1`.`room` LEFT JOIN `relatives_info` as relatives_info1 ON `relatives_info1`.`id`=`incoming_deceased1`.`relative_name` "
	);

	$pkey = array(
		'incoming_deceased' => 'id',
		'outgoing_deceased' => 'id',
		'relatives_info' => 'id',
		'rooms' => 'id',
		'beds' => 'id',
		'bill' => 'id',
...
function get_defaults($table)
{
	...
	$defaults = array(
		...
		'relatives_info' => array(
			'id' => '',
			'first_relative_full_name' => '',
			'home_address' => '',
			'home_town' => '',
			'occupation' => '',
			'phone_number' => '',
			'second_relative_full_name' => '',
			'second_relative_home_address' => '',
			'second_relative_home_town' => '',
			'second_relative_occupation' => '',
...
function logInMember()
{
	...
	if ($_POST['signIn'] != '') {
		if ($_POST['username'] != '' && $_POST['password'] != '') {
			...
			if (sqlValue("select count(1) from membership_users where lcase(memberID)='$username' and passMD5='$password' and isApproved=1 and isBanned=0") == 1) {
				...
				if ($_POST['rememberMe'] == 1) {
					@setcookie('Morgue_Management_rememberMe', md5($username . $password), time() + 86400 * 30);
				} else {
					@setcookie('Morgue_Management_rememberMe', '', time() - 86400 * 30);
				}
		...
		exit;
	} elseif ((!$_SESSION['memberID'] || $_SESSION['memberID'] == $adminConfig['anonymousMember']) && $_COOKIE['Morgue_Management_rememberMe'] != '') {
		$chk = makeSafe($_COOKIE['Morgue_Management_rememberMe']);
